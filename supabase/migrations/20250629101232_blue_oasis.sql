/*
  # Sample Data for PKP Monitor System

  1. New Tables Data
    - `puskesmas` - 5 sample puskesmas in Sukabumi
    - `bundles` - Bundle PKP 2024 with 6 klasters
    - `klasters` - 6 klasters for different health areas
    - `indikators` - Mix of scoring and target achievement indicators
    - `scoring_indikators` - Scoring criteria for scoring type indicators
    - `target_achievement_indikators` - Target details for achievement type indicators

  2. Data Structure
    - Each klaster contains 2-3 indicators
    - Mix of scoring (0,4,7,10) and target achievement indicators
    - Both annual and monthly periodicity for targets
    - Realistic health program indicators

  3. Conflict Handling
    - All inserts use conflict resolution to prevent duplicates
...
BEGIN
    SELECT b.id INTO bundle_2024_id FROM bundles b WHERE b.tahun = 2024;
    
    -- Check if klasters already exist for this bundle
    SELECT EXISTS(SELECT 1 FROM klasters k WHERE k.bundle_id = bundle_2024_id) INTO klaster_exists;
    ...
    -- Get klaster IDs
    SELECT k.id INTO klaster_1_id FROM klasters k WHERE k.bundle_id = bundle_2024_id AND k.nama_klaster LIKE 'Klaster 1:%';
    SELECT k.id INTO klaster_2_id FROM klasters k WHERE k.bundle_id = bundle_2024_id AND k.nama_klaster LIKE 'Klaster 2:%';
    SELECT k.id INTO klaster_3_id FROM klasters k WHERE k.bundle_id = bundle_2024_id AND k.nama_klaster LIKE 'Klaster 3:%';
    SELECT k.id INTO klaster_4_id FROM klasters k WHERE k.bundle_id = bundle_2024_id AND k.nama_klaster LIKE 'Klaster 4:%';
    SELECT k.id INTO klaster_5_id FROM klasters k WHERE k.bundle_id = bundle_2024_id AND k.nama_klaster LIKE 'Klaster 5:%';
    SELECT k.id INTO klaster_6_id FROM klasters k WHERE k.bundle_id = bundle_2024_id AND k.nama_klaster LIKE 'Klaster 6:%';
    ...
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_1_id AND i.nama_indikator = 'Rencana 5 Tahunan');
    
    INSERT INTO indikators (klaster_id, nama_indikator, definisi_operasional, type, urutan) 
    SELECT klaster_1_id, 'Kegiatan Pos Bindu PTM', 'Jumlah pos bindu PTM yang aktif melakukan kegiatan rutin', 'scoring', 2
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_1_id AND i.nama_indikator = 'Kegiatan Pos Bindu PTM');
    ...
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_1_id AND i.nama_indikator = 'Cakupan Penyuluhan Kesehatan');
    ...
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_2_id AND i.nama_indikator = 'Cakupan Inspeksi Sanitasi TPM');
    
    INSERT INTO indikators (klaster_id, nama_indikator, definisi_operasional, type, urutan) 
    SELECT klaster_2_id, 'Pembinaan TUPM', 'Jumlah tempat umum dan pengelolaan makanan yang dibina per bulan', 'target_achievement', 2
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_2_id AND i.nama_indikator = 'Pembinaan TUPM');
    ...
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_3_id AND i.nama_indikator = 'Cakupan K4');
    
    INSERT INTO indikators (klaster_id, nama_indikator, definisi_operasional, type, urutan) 
    SELECT klaster_3_id, 'Kunjungan Neonatal', 'Jumlah kunjungan neonatal per bulan', 'target_achievement', 2
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_3_id AND i.nama_indikator = 'Kunjungan Neonatal');
    ...
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_4_id AND i.nama_indikator = 'Cakupan Balita Ditimbang');
    
    INSERT INTO indikators (klaster_id, nama_indikator, definisi_operasional, type, urutan) 
    SELECT klaster_4_id, 'Distribusi PMT Balita', 'Distribusi PMT untuk balita gizi kurang per bulan', 'target_achievement', 2
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_4_id AND i.nama_indikator = 'Distribusi PMT Balita');
    ...
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_5_id AND i.nama_indikator = 'Cakupan Penemuan TB');
    
    INSERT INTO indikators (klaster_id, nama_indikator, definisi_operasional, type, urutan) 
    SELECT klaster_5_id, 'Pemeriksaan Kontak TB', 'Jumlah pemeriksaan kontak TB per bulan', 'target_achievement', 2
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_5_id AND i.nama_indikator = 'Pemeriksaan Kontak TB');
    ...
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_6_id AND i.nama_indikator = 'Waktu Tunggu Pelayanan');
    
    INSERT INTO indikators (klaster_id, nama_indikator, definisi_operasional, type, urutan) 
    SELECT klaster_6_id, 'Kepuasan Pasien', 'Tingkat kepuasan pasien terhadap pelayanan', 'scoring', 2
    WHERE NOT EXISTS (SELECT 1 FROM indikators i WHERE i.klaster_id = klaster_6_id AND i.nama_indikator = 'Kepuasan Pasien');

    -- Insert scoring details for scoring type indikators with conflict handling
    
    -- Rencana 5 Tahunan
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_1_id AND i.nama_indikator = 'Rencana 5 Tahunan';
    IF current_indikator_id IS NOT NULL THEN
        ...
        WHERE NOT EXISTS (SELECT 1 FROM scoring_indikators s WHERE s.indikator_id = current_indikator_id);
    END IF;

    -- Kegiatan Pos Bindu PTM
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_1_id AND i.nama_indikator = 'Kegiatan Pos Bindu PTM';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO scoring_indikators (indikator_id, skor_0, skor_4, skor_7, skor_10) 
    ...
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_6_id AND i.nama_indikator = 'Waktu Tunggu Pelayanan';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO scoring_indikators (indikator_id, skor_0, skor_4, skor_7, skor_10) 
    ...
    -- Kepuasan Pasien
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_6_id AND i.nama_indikator = 'Kepuasan Pasien';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO scoring_indikators (indikator_id, skor_0, skor_4, skor_7, skor_10) 
    ...
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_1_id AND i.nama_indikator = 'Cakupan Penyuluhan Kesehatan';
    IF current_indikator_id IS NOT NULL THEN
        ...
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators t WHERE t.indikator_id = current_indikator_id);
    END IF;

    -- Cakupan Inspeksi Sanitasi TPM (Annual)
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_2_id AND i.nama_indikator = 'Cakupan Inspeksi Sanitasi TPM';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
    ...
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_2_id AND i.nama_indikator = 'Pembinaan TUPM';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
    ...
    -- Cakupan K4 (Annual)
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_3_id AND i.nama_indikator = 'Cakupan K4';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
    ...
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_3_id AND i.nama_indikator = 'Kunjungan Neonatal';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
    ...
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_4_id AND i.nama_indikator = 'Cakupan Balita Ditimbang';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
    ...
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_4_id AND i.nama_indikator = 'Distribusi PMT Balita';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
    ...
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_5_id AND i.nama_indikator = 'Cakupan Penemuan TB';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
    ...
    SELECT i.id INTO current_indikator_id FROM indikators i WHERE i.klaster_id = klaster_5_id AND i.nama_indikator = 'Pemeriksaan Kontak TB';
    IF current_indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
