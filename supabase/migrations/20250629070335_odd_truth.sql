...
BEGIN
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO scoring_indikators (indikator_id, skor_0, skor_4, skor_7, skor_10) 
        SELECT indikator_id, 'Tidak ada rencana 5 tahunan', 'Ada rencana tapi tidak sesuai visi misi', 'Sesuai visi misi tapi belum berbasis analisis kebutuhan', 'Sesuai visi misi dan berbasis analisis kebutuhan masyarakat'
        WHERE NOT EXISTS (SELECT 1 FROM scoring_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO scoring_indikators (indikator_id, skor_0, skor_4, skor_7, skor_10) 
        SELECT indikator_id, 'Tidak ada pos bindu yang aktif', '1-2 pos bindu aktif', '3-4 pos bindu aktif', 'Lebih dari 4 pos bindu aktif'
        WHERE NOT EXISTS (SELECT 1 FROM scoring_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO scoring_indikators (indikator_id, skor_0, skor_4, skor_7, skor_10) 
        SELECT indikator_id, 'Lebih dari 60 menit', '31-60 menit', '16-30 menit', 'Kurang dari 15 menit'
        WHERE NOT EXISTS (SELECT 1 FROM scoring_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO scoring_indikators (indikator_id, skor_0, skor_4, skor_7, skor_10) 
        SELECT indikator_id, 'Kurang dari 60%', '60-74%', '75-89%', '90% atau lebih'
        WHERE NOT EXISTS (SELECT 1 FROM scoring_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        ...
        SELECT indikator_id, 80, 20, 'kegiatan', 'monthly'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
        SELECT indikator_id, 90, 150, 'TPM', 'annual'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
        SELECT indikator_id, 85, 50, 'tempat', 'monthly'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
        SELECT indikator_id, 95, 500, 'ibu hamil', 'annual'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
        SELECT indikator_id, 95, 40, 'kunjungan', 'monthly'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
        SELECT indikator_id, 85, 800, 'balita', 'annual'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
        SELECT indikator_id, 90, 25, 'balita', 'monthly'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
        SELECT indikator_id, 70, 100, 'kasus', 'annual'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
    ...
    IF indikator_id IS NOT NULL THEN
        INSERT INTO target_achievement_indikators (indikator_id, target_percentage, total_sasaran, satuan, periodicity) 
        SELECT indikator_id, 80, 15, 'pemeriksaan', 'monthly'
        WHERE NOT EXISTS (SELECT 1 FROM target_achievement_indikators WHERE indikator_id = indikator_id);
    END IF;
